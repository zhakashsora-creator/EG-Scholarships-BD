...
export default function DashboardClient({ user, signOutPath, initialTab = "overview" }: { user: { name: string; email: string }; signOutPath: string; initialTab?: string }) {
  ...
  const [priorityMatches, setPriorityMatches] = useState<ScholarshipMatch[]>([]);
  ...
  useEffect(() => {
    Promise.all([fetch("/api/workspace").then(readJson), fetch("/api/documents").then(readJson)])
      .then(([workspace, documentData]) => {
        ...
        setPriorityMatches(workspace.priorityMatches ?? []);
  ...
  async function runMatch() {
    ...
    try {
      ...
      const payload = await readJson(await fetch("/api/analyze", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ profile, localExtraction, emailReport: reportEmailConfigured && emailReport, followUpConsent }) }));
      setProfile({ ...emptyProfile, ...payload.profile });
      setMatches(payload.results ?? []);
      setPriorityMatches(payload.priorityMatches ?? []);
  ...
  const pageDescriptions: Record<Tab, string> = {
    ...
    matches: "Explore every relevant available result, filter the catalogue and compare fully funded build-up targets.",
...
function AccountSetup({ account, email, busy, notice, onSave, signOutPath }: { account: AccountProfile; email: string; busy: boolean; notice: string; onSave: (data: FormData) => Promise<boolean>; signOutPath: string }) {
  return <main className="onboarding-shell">
    ...
    <section className="onboarding-card">
      <div className="onboarding-intro"><span className="section-kicker">WELCOME TO YOUR WORKSPACE</span><h1>Let’s set up your student account.</h1><p>These details help EG Consultancy identify your profile and deliver your personal scholarship report. Only your name, address and Bangladesh mobile number are required.</p><ol><li className="active"><b>1</b><span><strong>Account details</strong><small>Tell us who you are</small></span></li><li><b>2</b><span><strong>Study profile</strong><small>Add academic preferences</small></span></li><li><b>3</b><span><strong>All matches + report</strong><small>Review every relevant option</small></span></li></ol></div>
...
function Profile({ profile, setProfile, consent, setConsent, emailReport, setEmailReport, reportEmailConfigured, followUpConsent, setFollowUpConsent, email, documents, busy, analysisProgress, aiConfigured, localReadingSupported, onSave, onRun }: { profile: StudentProfile; setProfile: (profile: StudentProfile) => void; consent: boolean; setConsent: (value: boolean) => void; emailReport: boolean; setEmailReport: (value: boolean) => void; reportEmailConfigured: boolean; followUpConsent: boolean; setFollowUpConsent: (value: boolean) => void; email: string; documents: number; busy: boolean; analysisProgress: string; aiConfigured: boolean; localReadingSupported: boolean; onSave: () => void; onRun: () => void }) {
  ...
  return <section className="panel profile-panel expanded-profile">
    ...
    <section className="report-delivery-choice" aria-labelledby="report-delivery-title"><div className="report-choice-head"><span>PDF</span><div><strong id="report-delivery-title">Your detailed EG Consultancy report</strong><p>Alongside the complete results shown here, we prepare a branded PDF with your profile summary, fully funded priorities, match realities, official links and next-step plan.</p></div></div>{reportEmailConfigured ? <label htmlFor="email-report"><input id="email-report" name="emailReport" type="checkbox" checked={emailReport} onChange={(event) => setEmailReport(event.target.checked)} disabled={busy} /><span><strong>Email the report to {email}</strong><small>You can also download it securely from Best Finds.</small></span></label> : <p className="report-download-only">Email delivery is unavailable right now. Your report will still be available as a secure PDF download.</p>}<label htmlFor="follow-up-consent"><input id="follow-up-consent" name="followUpConsent" type="checkbox" checked={followUpConsent} onChange={(event) => setFollowUpConsent(event.target.checked)} disabled={busy} /><span><strong>EG Consultancy may contact me about this shortlist</strong><small>Optional. This helps a consultant follow up about an office consultation.</small></span></label></section>
    <div className="profile-actions"><button className="button ghost" onClick={onSave} disabled={busy}>{busy ? "Working..." : "Save profile"}</button><button className="button primary match-button" onClick={onRun} disabled={busy}>{busy ? analysisProgress || "Finding opportunities..." : reportEmailConfigured && emailReport ? "Find all matches & email report" : "Find all matches"}<span>✦</span></button></div>
  </section>;
}
...
function Matches({ matches, priorityMatches, countryNotices, profile, report, busy, onRetryReport, onProfile, onConsultant, onTrack }: { matches: ScholarshipMatch[]; priorityMatches: ScholarshipMatch[]; countryNotices: CountryCoverageNotice[]; profile: StudentProfile; report: ReportDelivery | null; busy: boolean; onRetryReport: () => void; onProfile: () => void; onConsultant: () => void; onTrack: (id: string) => void }) {
  ...
  const [freshness, setFreshness] = useState("All records");
  ...
  const countries = useMemo(() => Array.from(new Set(matches.map((match) => match.scholarship.country))).sort(), [matches]);
  const filtered = useMemo(() => matches.filter((match) => {
    const text = `${match.scholarship.name} ${match.scholarship.provider} ${match.scholarship.country} ${match.scholarship.subjectRestrictions}`.toLowerCase();
    return (country === "All countries" || match.scholarship.country === country)
      && (funding === "All funding" || fundingCategory(match.scholarship) === funding)
      && (band === "All bands" || match.label === band)
      && (freshness === "All records" || (freshness === "Recently verified" ? verificationAgeDays(match.scholarship) <= 60 : verificationAgeDays(match.scholarship) > 60))
      && (!query.trim() || text.includes(query.trim().toLowerCase()));
  }).sort((left, right) => sort === "deadline"
    ? (left.scholarship.deadline || "9999-99-99").localeCompare(right.scholarship.deadline || "9999-99-99")
    : sort === "country"
      ? left.scholarship.country.localeCompare(right.scholarship.country) || right.score - left.score
      : right.score - left.score), [matches, country, funding, band, freshness, query, sort]);
  ...
  return <>{report && <section className={`report-status-card ${report.status}`}><div className="report-status-icon">{report.status === "sent" ? "✓" : "PDF"}</div><div><span className="section-kicker">YOUR PERSONAL SCHOLARSHIP REPORT</span><h2>{report.status === "sent" ? "Your report is in your inbox." : "Your report is ready."}</h2><p>{report.message} It includes your profile summary, fully funded priority options, reality checks, an action plan and the consultation bridge.</p></div><div className="report-status-actions"><a className="button primary compact" href={report.downloadUrl}>Download PDF</a>{report.status === "failed" && report.emailConfigured && <button className="button ghost compact" onClick={onRetryReport} disabled={busy}>{busy ? "Sending..." : "Retry email"}</button>}</div></section>}<div className="match-notice"><span>i</span><p>Scores guide prioritization; they do not guarantee eligibility, admission, funding or a visa. Recheck the official source before acting.</p></div>{countryNotices.length > 0 && <section className="country-gap-list" aria-label="Destination coverage notices">{countryNotices.map((item) => <article key={item.country}><strong>{item.message}</strong><ul>{item.alternatives.map((alternative) => <li key={alternative}>{alternative}</li>)}</ul></article>)}</section>}<div className="finds-heading"><div><span className="section-kicker">FULLY FUNDED · PROFILE BUILD-UP TARGETS</span><h2>{priorityMatches.length} priority option{priorityMatches.length === 1 ? "" : "s"}</h2><p>Aspirational fully funded choices to target with stronger evidence, English scores, experience and applications. Some may be outside your selected destinations.</p></div><div className="carousel-controls"><button type="button" onClick={() => move(-1)} aria-label="Previous opportunity">←</button><button type="button" onClick={() => move(1)} aria-label="Next opportunity">→</button></div></div>
    <div className="best-finds-rail" ref={rail} tabIndex={0} aria-label="Fully funded priority scholarship carousel">{priorityMatches.map((match, index) => <MatchCard key={match.scholarship.id} match={match} index={index} onTrack={onTrack} onCourses={setCourseTarget} />)}</div>
    <section className="match-catalogue"><div className="finds-heading"><div><span className="section-kicker">ALL AVAILABLE PROFILE MATCHES</span><h2>{filtered.length} of {matches.length} result{matches.length === 1 ? "" : "s"}</h2><p>No ten-result cap. Use the filters to narrow the complete relevant catalogue for this profile.</p></div></div><div className="match-filters"><label>Search<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Scholarship, provider or subject" /></label><label>Country<select value={country} onChange={(event) => setCountry(event.target.value)}><option>All countries</option>{countries.map((item) => <option key={item}>{item}</option>)}</select></label><label>Funding<select value={funding} onChange={(event) => setFunding(event.target.value)}><option>All funding</option>{["Fully funded", "Full tuition", "Partial funding", "Tuition discount", "Other"].map((item) => <option key={item}>{item}</option>)}</select></label><label>Match band<select value={band} onChange={(event) => setBand(event.target.value)}><option>All bands</option>{["Strong match", "Possible match", "Reach"].map((item) => <option key={item}>{item}</option>)}</select></label><label>Verification<select value={freshness} onChange={(event) => setFreshness(event.target.value)}><option>All records</option><option>Recently verified</option><option>Needs recheck</option></select></label><label>Sort<select value={sort} onChange={(event) => setSort(event.target.value)}><option value="score">Best score</option><option value="deadline">Nearest deadline</option><option value="country">Country</option></select></label></div>{filtered.length ? <div className="all-results-grid">{filtered.map((match, index) => <MatchCard compact key={match.scholarship.id} match={match} index={index} onTrack={onTrack} onCourses={setCourseTarget} />)}</div> : <div className="filter-empty"><strong>No results meet every selected filter.</strong><button type="button" onClick={() => { setCountry("All countries"); setFunding("All funding"); setBand("All bands"); setFreshness("All records"); setQuery(""); }}>Clear filters</button></div>}</section>
    <section className="top-five-analysis"><div className="finds-heading"><div><span className="section-kicker">PRIORITY SHORTLIST · DECISION DETAIL</span><h2>Profile fit and cost planning</h2><p>Open a fully funded target to review its profile comparison and planning breakdown.</p></div></div>{priorityMatches.map((match, index) => { const fitChecks = buildFitChecks(profile, match.scholarship); const costPlan = buildCostPlan(match.scholarship); return <details key={match.scholarship.id} open={index === 0}><summary><b>#{index + 1}</b><span><strong>{match.scholarship.name}</strong><small>{match.scholarship.country} · {match.score}% match</small></span><i>＋</i></summary><div className="top-five-detail-body"><section><span className="section-kicker">PROFILE VS REQUIREMENTS</span><div className="compact-fit-grid">{fitChecks.map((check) => <article key={check.label}><b>{check.label}</b><span>{check.student}</span><p>{check.requirement}</p><i className={check.status.toLowerCase().replace(" ", "-")}>{check.status}</i></article>)}</div></section><section><span className="section-kicker">DETAILED COST PLAN</span><div className="compact-cost-grid">{costPlan.map((cost) => <article key={cost.item}><b>{cost.item}</b><span>{cost.awardPosition}</span><p>{cost.planningAction}</p></article>)}</div></section><Link className="button primary compact" href={`/dashboard/scholarship/${encodeURIComponent(match.scholarship.id)}`}>Open full match analysis →</Link></div></details>; })}</section>
...
function MatchCard({ match, index, compact = false, onTrack, onCourses }: { match: ScholarshipMatch; index: number; compact?: boolean; onTrack: (id: string) => void; onCourses: (match: ScholarshipMatch) => void }) {
  const stale = verificationAgeDays(match.scholarship) > 60;
  return <article className={`find-card ${compact ? "compact-result" : ""}`}><div className="find-cover"><img src={campusImage(match.scholarship.provider)} alt={`${match.scholarship.provider} campus`} loading="lazy" /><span>{match.scholarship.country}</span><b>#{index + 1}</b><div className="find-score"><strong>{match.score}</strong><small>match</small></div></div><div className="find-body"><span className={`match-label ${match.label === "Strong match" ? "strong" : match.label === "Reach" ? "reach" : ""}`}>{match.label}</span><h3>{match.scholarship.name}</h3><p className="provider">{match.scholarship.provider}</p><div className="match-meta"><span>{match.scholarship.studyLevel}</span><span>{fundingCategory(match.scholarship)}</span></div><dl className="match-subscores"><div><dt>Subject</dt><dd>{match.subScores.subjectFit}/20</dd></div><div><dt>English</dt><dd>{match.subScores.englishHeadroom}/14</dd></div><div><dt>Level</dt><dd>{match.subScores.studyLevel}/18</dd></div><div><dt>Eligibility</dt><dd>{match.subScores.eligibility}/16</dd></div></dl><p className="find-rationale">{match.rationale}</p>{match.gaps.length > 0 && <div className="find-gap"><b>Check:</b> {match.gaps[0]}</div>}<div className={`verification-date ${stale ? "stale" : ""}`}><span>Catalogue checked {match.scholarship.verifiedAt || "date unavailable"}</span><strong>{stale ? "Re-verify now" : /high/i.test(match.scholarship.confidence ?? "") ? "High confidence" : "Lower confidence · verify"}</strong></div><div className="find-deadline"><span>Deadline / cycle</span><strong>{match.scholarship.deadline || "Annual / rolling"}</strong></div><button className="course-discovery-button" type="button" onClick={() => onCourses(match)}><span>✦</span> Find subjects & courses <b>Official links</b></button></div><footer><Link className="match-analysis-button" href={`/dashboard/scholarship/${encodeURIComponent(match.scholarship.id)}`}>View full match analysis</Link><button type="button" onClick={() => onTrack(match.scholarship.id)}>Track</button></footer></article>;
}
...
function ApplicationCard({ application, scholarship, inCurrentResults, recordCount, onSave }: { application: ApplicationItem; scholarship: Scholarship; inCurrentResults: boolean; recordCount: number; onSave: (id: string, stage: string, workflow?: ApplicationWorkflow) => void }) {
  ...
  return <article className="application-detail-card">
    <header><div><span>{scholarship.country}</span><h3>{scholarship.name}</h3>{!inCurrentResults && <small className="application-rank-note">No longer in your current top ten</small>}<p>{application.nextAction}</p></div><Link href={`/dashboard/scholarship/${encodeURIComponent(scholarship.id)}`}>View analysis →</Link></header>
    ...
    <footer><div className="application-record-links"><a href={scholarship.officialSource} target="_blank" rel="noreferrer">Check official source ↗</a><Link href="/dashboard?tab=documents">Store related email or receipt →</Link></div><button className="button primary compact" onClick={() => onSave(application.scholarshipId, stage, workflow)}>Save application plan</button></footer>
  </article>;
}
