...
const mysqlSchema = [
  ...
  `CREATE TABLE IF NOT EXISTS scholarship_catalogue (id VARCHAR(120) PRIMARY KEY, name VARCHAR(500) NOT NULL, provider VARCHAR(500) NOT NULL, country VARCHAR(160) NOT NULL, funding_category VARCHAR(80) NOT NULL, official_source VARCHAR(1500) NOT NULL, deadline VARCHAR(120), status VARCHAR(255), verified_at VARCHAR(80), confidence VARCHAR(80), source_dataset TEXT, data_json LONGTEXT NOT NULL, active TINYINT(1) NOT NULL DEFAULT 1, source_order INT NOT NULL DEFAULT 0, created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3), updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3), INDEX scholarship_catalogue_country_idx (country), INDEX scholarship_catalogue_funding_idx (funding_category), INDEX scholarship_catalogue_active_idx (active, source_order)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`,
  `ALTER TABLE scholarship_catalogue MODIFY COLUMN source_dataset TEXT`,
];
