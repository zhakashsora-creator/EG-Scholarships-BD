...
export async function buildScholarshipReportPdf(snapshot: ReportSnapshot, bookingUrl: string) {
  ...
  textLine(writer, `${snapshot.matches.length} ranked options are included. Strong matches should be verified first; Possible and Reach options need the stated gaps resolved before you invest in an application.`, { size: 10, color: MUTED, gap: 8 });
  ...
  snapshot.matches.forEach((match, index) => {
    ...
    textLine(writer, `Weighted score: subject ${match.subScores.subjectFit}/20 | English ${match.subScores.englishHeadroom}/14 | level ${match.subScores.studyLevel}/18 | eligibility ${match.subScores.eligibility}/16`, { size: 8.5, color: TEAL, gap: 5 });
